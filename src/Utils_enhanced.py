...
from src.EnhancedMHGCN import EnhancedMHGCN, EnhancedMHGCNWithContrastive
...
def load_our_data(dataset_str, cuda=True):
    ...
    # label
    try:
        labels = data['label']  # 节点的标签
    except:
        labels = data['labelmat']  # 节点的标签
    N = labels.shape[0]
    try:
...
def get_model(model_opt, nfeat, nclass, A, nhid, out, dropout=0, cuda=True):
    ...
    if model_opt == "MHGCN":
        model = MHGCN(nfeat=nfeat,
                     ...
                     out=out,
                     dropout=dropout)
    elif model_opt == "EnhancedMHGCN":
        model = EnhancedMHGCN(nfeat=nfeat,
                             nhid=nhid,
                             out=out,
                             dropout=dropout,
                             alpha=0.2,
                             nheads=8)
    elif model_opt == "EnhancedMHGCNWithContrastive":
        model = EnhancedMHGCNWithContrastive(nfeat=nfeat,
                                           nhid=nhid,
                                           out=out,
                                           dropout=dropout,
                                           alpha=0.2,
